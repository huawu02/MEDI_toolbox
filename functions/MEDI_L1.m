...
%     S = SMV_kernel(matrix_size, voxel_size,radius);
    ...
    Mask = SMV(Mask, SphereK)>0.999;   % HW: don't want to reduce the Mask? 
    D=(1-SphereK).*D;
...
m = dataterm_mask(data_weighting_mode, tempn, Mask); % HW: create a data weighting mask based on noise level in data
b0 = m.*exp(1i*RDF);
